...
export default function PlannerView({ recipes, profile, onAddPlan }) {
  ...
  return (
    <main className="planner-page">
      ...
      {result && <>
        ...
        <div className="plan-list">{result.items.map((recipe, index) => <article className="plan-row" key={recipe.id}>{recipeImages[recipe.id] ? <img src={recipeImages[recipe.id]} alt="" loading="lazy" decoding="async" /> : <span className="plan-index">{index + 1}</span>}<div><small>{["Breakfast / first meal", "Lunch / second meal", "Dinner / third meal"][index]}</small><strong>{recipe.name}</strong><span>{formatAmount(recipe.calories, 1)} kcal · {formatAmount(recipe.protein, 1)} g protein · {formatAmount(recipe.sodium, 1)} mg sodium</span></div></article>)}</div>
